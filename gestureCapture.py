...
while(1):
    ...
    try:  
        ...
        # getting the mean skin colour of 100 samples
        if outputCount > 10:
            # setting the output to a numpy array
            output = np.array(output)
            print(output.astype(int))
            # getting the mean hsv value
            meanOutput = output.mean(axis=0)
            # setting all values to whole numbers
            meanOutput = meanOutput.astype(int)
            # setting the upper and lower skin tone range

            # set the boolean value to true
            skinToneExtracted = 1

        # *************************************** Extracting Skin Tone ********************************************
        if skinToneExtracted == 0:
            ...
            for (x, y, w, h) in faces:
                ...
                if roi is not None:
                    ...
                    face2DArray = np.array(face2DArray)
                    # calculate the mean hsv values for the 2D array
                    mean = face2DArray.mean(axis=0)
                    # calculate the mean of the mean, which gives us a single hsv value
                    mean = mean.mean(axis=0)
                    # append the hsv value to the output array
                    output.append(mean)
                    outputCount += 1
                    print(outputCount)
                    print(face1DArray)
